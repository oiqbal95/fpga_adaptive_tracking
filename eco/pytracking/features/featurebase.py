...
class FeatureBase:
    ...
    """
    def __init__(self, fparams = None, pool_stride = [2,1], output_size = None, normalize_power = 2, use_for_color = True, use_for_gray = True):
    ...
    def get_feature(self, im: torch.Tensor,dpu_features):
...
class MultiFeatureBase(FeatureBase):
    ...
    def get_feature(self, im: torch.Tensor,dpu_features):
        ...
        #print(dpu_features.dtype)
        #print("[INFO][featurebase.py] dpu_features[0][0][0][0]:\n", dpu_features[0][0][0][0])
        feat_list = TensorList(dpu_features)
        #print("[INFO][featurebase.py] feat_list[0][0][0][0]: \n", feat_list[0][0][0][0])
        #np.save('hw_feat_list1',feat_list[0][0][0])
        
        #print("FEAT_LIST")
        #print(feat_list.dtype)
        output_sz = [None]*len(feat_list) if self.output_size is None else self.output_size

        #print("[INFO][featurebase.py] feat_list[0] shape: ", feat_list[0].shape)
        #print("[INFO][featurebase.py] feat_list[1] shape: ", feat_list[1].shape)
        ...
        if self.normalize_power is not None:
            for feat in feat_list:
                #print("CHECK FEAT ABS SHAPE")
                #print(feat.abs().shape)
                cc = feat.contiguous().abs()
                feat /= (torch.sum(cc.view(feat.shape[0],1,1,-1)**self.normalize_power, dim=3, keepdim=True) /
                         (feat.shape[1]*feat.shape[2]*feat.shape[3]) + 1e-10)**(1/self.normalize_power)
       
        #print("[INFO][featurebase.py] feat_list[0][0][0][0]\n", feat_list[0][0][0][0])
        #np.save('hw_feat_list2',feat_list[0][0][0])
