...
class MultiResolutionExtractor(ExtractorBase):
    ...
    def extract(self, dpu_features,im, pos, scales, image_sz, return_patches=False):
        """Extract features.
        args:
            im: Image.
        ...
        #print("[INFO][extractor.py] pos: ", pos)
        #print("[INFO][extractor.py] scales: ", scales)
        #print("[INFO][extractor.py] image_sz: ", image_sz)
        ...
        # im_patches = torch.cat([sample_patch(im, pos, s*image_sz, image_sz) for s in scales])
        #print("[INFO][extractor.py] im_patches[0][0][0]: ", im_patches[0][0][0])

        # Compute features
        feature_map = TensorList([f.get_feature(im_patches,dpu_features) for f in self.features]).unroll()
        #print("[INFO][extractor.py] feature_map shape: ", feature_map[0][0][0][0])
    ...
    def extract_transformed(self, im, pos, scale, image_sz, transforms, dpu_features):
        ...
        #print("[INFO][extractor.py] im_patches: ", im_patches[0][0][0])
        

        # Compute features
        feature_map = TensorList([f.get_feature(im_patches,dpu_features) for f in self.features]).unroll()
        #feature_map = TensorList([dpu_features]).unroll()

        #print("[INFO][extractor.py] feature_map shape: ", feature_map[0].shape)
        #print("[INFO][extractor.py] feature_map: \n", feature_map[0][0][0])
