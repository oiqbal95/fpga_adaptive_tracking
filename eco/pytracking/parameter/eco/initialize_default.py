...
def parameters():
    ...
    #features = deep.ResNet18m1(output_layers=['layer1', 'layer2', 'layer3', 'layer4'], use_gpu=params.use_gpu, fparams=fparams,
    #                           pool_stride=[2, 1], normalize_power=2)
    #print("PARAMS FEATURES")
    #print(features.size)
    #array_path = os.path.join('results', str(1).zfill(4)+'layer'+str(1)+'.npy') 
      
    #result = np.load(array_path)
    #params.features = MultiResolutionExtractor([features])
    #params.features = result
    #print(params.features)
    return params
