...
def cfft2(a):
    ...
    #a = np.array(a)
    a_fft = torch.from_numpy(np.fft.rfft2(a))
    a_fft = torch.view_as_real(a_fft)
    #print("[INFO][fourier.py] a_fft: ", a_fft[0])
    #brea
    a_fft = rfftshift2(a_fft)

    #print("[INFO][fourier.py] a_fft: \n", a_fft[0][0][0])
    return a_fft
...
def cifft2(a, signal_sizes=None):
    ...
    #a = np.array(a)
    #print("[INFO][fourier.py] a1 shape: ", a.shape)
    a = irfftshift2(a)
    #print("[INFO][fourier.py] a: ", a.shape)
    a = torch.view_as_complex(a)
    #print("[INFO][fourier.py] a2 shape: ", a.shape)
    a_irfft = torch.from_numpy(np.fft.irfft2(a))
    #return torch.irfft(irfftshift2(a), 2, signal_sizes=signal_sizes)
    #print("[INFO][fourier.py] a_irfft before shape: ", a_irfft.shape)
    #a_irfft = torch.view_as_real(a_irfft)
    #print("[INFO][fourier.py] a_irfft after shape: ", a_irfft.shape)
