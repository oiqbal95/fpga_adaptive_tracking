...
def get_reg_filter(sz: torch.Tensor, target_sz: torch.Tensor, params):
    #print("[INFO][dcf.py] get_reg_filter called")
    ...
    #Convert to numpy to use np.fft
    reg_window_dft = torch.from_numpy(np.fft.rfft2(reg_window)) / sz.prod()
        
    #Convert to real to add a dimension ([x, y] instead of [x+yj])
    reg_window_dft = torch.view_as_real(reg_window_dft)
    ...
    reg_window_sparse = np.fft.irfft(reg_window_dft)
    #Convert back to tensor
    reg_window_sparse = torch.from_numpy(reg_window_sparse)
