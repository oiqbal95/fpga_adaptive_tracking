...
import torch
import timeit
from pytracking.evaluation.kalman_filter import KFilter
...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        print("HEREEEE") 
        #print(out.get('target_bbox'))
        x = int(init_info.get('init_bbox')[0])
        y = int(init_info.get('init_bbox')[1])
        w = int(init_info.get('init_bbox')[2])
        h = int(init_info.get('init_bbox')[3])
        print('HERE')
        keyframe_interval = 0
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            #measure_time.append(abs(end-start))
            #print(end - start)

                end = timeit.timeit()
                x = int(out.get('target_bbox')[0])
                y = int(out.get('target_bbox')[1])
                w = int(out.get('target_bbox')[2])
                h = int(out.get('target_bbox')[3])
                prev_output = OrderedDict(out)
                ...
                det_box.w = w
                det_box.h = h
                        #predicted_bbox = kf.bbox(int(x_list[j]),int(y_list[j]),int(w_list[j]),int(h_list[j]))
                updated_bbox = kf.update(det_box)
            else:
                #orig_image = self._read_image(frame_path)
                #image = np.zeros([image.shape[0],image.shape[1],image.shape[2]],dtype=np.uint8)
                #image[y:y+h,x:x+w,:] = orig_image[y:y+h,x:x+w,:]
        ...
        for key in ['target_bbox', 'segmentation']:
            if key in output and len(output[key]) <= 1:
                output.pop(key)
        #print("HEERRRRRREEEEEEEEE")
        #print(output.get('target_bbox'))
        return output
    ...
    def _read_image(self, image_file: str):
        im_main = cv.imread(image_file)
        im = cv.resize(im_main,(448,448))
        #print(image_file)
