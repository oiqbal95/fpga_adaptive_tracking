...
if __name__ == '__main__':
    ...
    for frame_num in range(total_frames):
        ...
        if frame_num % key_frame == 0:
            ...
            img = [np.array(yolo_preprocessing(image), dtype=np.float32)]
        ...
        #Timing: image draw and showstart
        tic = time.perf_counter()
        ...
        #Timing: image draw and show end
        toc = time.perf_counter()
        imageDrawShow += toc - tic
    ...
    print("Average image show time:     %.6f" % (imageDrawShow / total_frames ) )
    print(divider)
